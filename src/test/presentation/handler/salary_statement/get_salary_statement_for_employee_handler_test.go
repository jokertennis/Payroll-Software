...
func TestMain(m *testing.M) {
	...
	err := createDataForTestsInSalaryStatementPackage(ctx, dbInstance)
	if err != nil {
		fmt.Println(err)
	}
...
func createDataForTestsInSalaryStatementPackage(ctx context.Context, db boil.ContextExecutor) error {
	...
	salary_statements := []models.SalaryStatement{
		...
		// {ID: 5, EmployeeID: 9, Nominal: "2010年2月 給与明細", Payday: time.Date(2010, 2, 20, 0, 0, 0, 0, time.Local), TargetPeriod: "2010年1月1日~2010年1月31日"}}
...
func TestGetSalaryStatementForEmployeeHandler(t *testing.T) {
	...
	for _, value := range cases {
		...
		// assert.Equal(t, "a", req.URL.String())
