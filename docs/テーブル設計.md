...
| 個別支給id | individual_earning_id  | MEDIUMINT(UNSIGNED)  |  | ◯ | ◯ |  |
| 固定支給id  | fixed_earning_id  | MEDIUMINT(UNSIGNED)  |  |  | ◯ |  |
| 個別控除id | individual_deduction_id  | MEDIUMINT(UNSIGNED)  |  | ◯ | ◯ |  |
...
・個別支給idと固定支給idどちらもnullは許可しない。また、どちらにも値が入ることは許可しない。
・個別控除idと固定控除idどちらもnullは許可しない。また、どちらにも値が入ることは許可しない。

## 個別支給テーブル(individual_earnings_table)
| 名称  | 変数名 | 型  | NOT NULL | UNIQUE | INDEX | 説明
| ------------- | ------------- | ------------- | ------------- | ------------- | ------------- | ------------- |
| 個別支給id  | id  | MEDIUMINT(UNSIGNED) | ◯  | ◯ | ◯ | 主キーでauto incrementされる |
| 名目  | nominal  | VARCHAR(255) | ◯ |  |  | 給与明細書の支給詳細の名目を表す。例)基本給、通勤手当など |
| 支給総額 | amount | INT  | ◯ |  |  | |

## 個別支給詳細テーブル(individual_earning_details_table)
| 名称  | 変数名 | 型  | NOT NULL | UNIQUE | INDEX | 説明
| ------------- | ------------- | ------------- | ------------- | ------------- | ------------- | ------------- |
| 個別支給詳細id  | id  | MEDIUMINT(UNSIGNED)  | ◯  | ◯ | ◯ | 主キーでauto incrementされる |
| 個別支給id  | individual_earning_id  | MEDIUMINT(UNSIGNED) | ◯  |  | ◯ |  |
| 名目  | nominal  | VARCHAR(255) | ◯ |  |  | 給与明細書の支給詳細の名目を表す。例)基本給、通勤手当など |
| 支給額  | amount | INT | ◯ |  |  | |
...
## 固定支給詳細テーブル(fixed_earning_details_table)
| 名称  | 変数名 | 型  | NOT NULL | UNIQUE | INDEX | 説明
| ------------- | ------------- | ------------- | ------------- | ------------- | ------------- | ------------- |
...
## 個別控除テーブル(individual_deductions_table)
| 名称  | 変数名 | 型  | NOT NULL | UNIQUE | INDEX | 説明
| ------------- | ------------- | ------------- | ------------- | ------------- | ------------- | ------------- |
| 個別控除id  | id  | MEDIUMINT(UNSIGNED) | ◯  | ◯ | ◯ | 主キーでauto incrementされる |
| 控除総額 | amount | INT | ◯ |  |  | |
| 名目  | nominal  | VARCHAR(255) | ◯ |  |  |  |

## 個別控除詳細テーブル(individual_deduction_details_table)
| 名称  | 変数名 | 型  | NOT NULL | UNIQUE | INDEX | 説明
| ------------- | ------------- | ------------- | ------------- | ------------- | ------------- | ------------- |
| 個別控除詳細id  | id  | MEDIUMINT(UNSIGNED) | ◯  | ◯ | ◯ | 主キーでauto incrementされる |
| 個別控除id  | individual_deduction_id  | MEDIUMINT(UNSIGNED) | ◯  |  | ◯ |  |
...
- 給料明細は毎月同じ内容のデータであることが多い。そのため、固定支給テーブル・固定詳細支給テーブル・固定控除テーブル・固定控除詳細テーブルを用意して、データの再利用ができるようにした。固定のテーブルの利用に関しては、事前に作成したデータがそのまま再利用できる場合のみ利用する方針である。１部分でもデータが異なる場合は、固定テーブルを利用せずに個別支給テーブル・個別詳細支給テーブル・個別控除テーブル・個別控除詳細テーブルを利用する必要がある。再利用するデータは何でも良い。例えば、従業員それぞれに固定データを作成して使いまわしたり、会社の給与形態を反映させた(複数の従業員が利用可能な)データを固定テーブルに登録してもよい。
