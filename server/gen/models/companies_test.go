...
func testCompanyToManyEmployees(t *testing.T) {
	...
	slice := CompanySlice{&a}
	if err = a.L.LoadEmployees(ctx, tx, false, (*[]*Company)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
var (
	companyDBTypes = map[string]string{`ID`: `smallint`, `Name`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_              = bytes.MinRead
)
