...
func testEarningToManySalaryStatements(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := EarningSlice{&a}
	if err = a.L.LoadSalaryStatements(ctx, tx, false, (*[]*Earning)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testEarningToManyAddOpSalaryStatements(t *testing.T) {
	var err error
...
var (
	earningDBTypes = map[string]string{`ID`: `mediumint`, `Nominal`: `varchar`, `Amount`: `int`, `EarningType`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_              = bytes.MinRead
)
