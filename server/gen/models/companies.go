...
var CompanyWhere = struct {
	...
	ID:        whereHelperuint16{field: "`companies`.`id`"},
...
var (
	companyAllColumns            = []string{"id", "name", "created_at", "updated_at"}
	companyColumnsWithoutDefault = []string{"name", "created_at", "updated_at"}
...
func (o *Company) Employees(mods ...qm.QueryMod) employeeQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`employees`.`company_id`=?", o.ID),
	)
...
func (companyL) LoadEmployees(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCompany interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`employees`),
		qm.WhereIn(`employees.company_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func FindCompany(ctx context.Context, exec boil.ContextExecutor, iD uint16, selectCols ...string) (*Company, error) {
	companyObj := &Company{}
...
func CompanyExists(ctx context.Context, exec boil.ContextExecutor, iD uint16) (bool, error) {
	var exists bool
